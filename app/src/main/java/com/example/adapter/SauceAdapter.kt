...
class SauceAdapter(val factoryList: MutableList<String>,
                   val sauceList: MutableList<String>,
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        ...
        return SauceViewHolder(itemView)
    }

    override fun getItemCount(): Int = sauceList.size

    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        val viewHolder = SauceViewHolder(holder.itemView)
        viewHolder.bindTo(position)

    }

    inner class SauceViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bindTo(position: Int) {
            ...
            for (i in 0 until factoryList.size) {
                ...
                if (factoryList[i] == "SAUCE") {
                    ...
                    description.text =
                        FactoryGenerator().getFactory(factoryList[i])?.getSauce(sauceList[position])
                            ?.getSauceDescription()

                    calories.text = FactoryGenerator().getFactory(factoryList[i])?.getSauce(sauceList[position])
                        ?.getSauceCalories()

                    image.setImageDrawable(
                        ContextCompat.getDrawable(
                            context,
                            FactoryGenerator().getFactory(factoryList[i])?.getSauce(sauceList[position])?.getSauceImage()!!
                        )
                    )
